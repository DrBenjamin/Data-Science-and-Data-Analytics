...
library(RefManageR)
# Simplifying PDF detection to core knitr check (quarto passes through)
is_pdf <- knitr::is_latex_output()
BibOptions(
    check.entries = FALSE,
    bib.style = "authoryear",
    cite.style = "authoryear",
    style = if (is_pdf) "latex" else "html",
    no.print.fields = c()
)
bib_essential <- ReadBib("literature/essential_readings.bib", check = FALSE)
if (is_pdf) {
    print(bib_essential)
} else {
    formatted <- format(bib_essential)
    # Collapsing internal newlines so each entry stays a single bullet line
    formatted <- vapply(formatted, function(x) gsub("\\s*\\n\\s*", " ", x), character(1))
    cat("\n")
    for (i in seq_along(formatted)) cat("- ", formatted[i], "\n", sep = "")
    cat("\n")
}
```
...
bib_further <- ReadBib("literature/further_readings.bib", check = FALSE)
if (is_pdf) {
    print(bib_further)
} else {
    formatted <- format(bib_further)
    formatted <- vapply(formatted, function(x) gsub("\\s*\\n\\s*", " ", x), character(1))
    cat("\n")
    for (i in seq_along(formatted)) cat("- ", formatted[i], "\n", sep = "")
    cat("\n")
